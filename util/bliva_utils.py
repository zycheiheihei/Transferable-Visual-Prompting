...
from models.bliva.models import *
from models.bliva.processors import *
from models.bliva.models import load_model_and_preprocess
...
def load_model(llm_name:str, device='cuda')->BLIVAVicuna:
    model, vis_processors, _ = load_model_and_preprocess(name="bliva_vicuna", model_type="vicuna7b", is_eval=True, device=device)
    
    return model


def model_generate(model:BLIVAVicuna, samples:Dict[str,torch.Tensor|List[str]|str|None], **generate_kwargs):
...
def model_loss(model:BLIVAVicuna, samples:Dict[str,torch.Tensor|List[str]|str|None]):
...
def model_loglikelihood_for_postfixes(model:BLIVAVicuna, samples:Dict[str,torch.Tensor|List[str]|str|None], postfixes:List[str], normalize=True):
    image = samples["image"]
    with model.maybe_autocast():
    ...
    if isinstance(model, BLIVAVicuna):
